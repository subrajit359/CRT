...
router.post("/", requireAuth(), async (req, res) => {
  try {
    ...
    if (!isPractice && req.user.role === "student" && score !== null) {
      // ── Infinite level-up system ───────────────────────────────────────────
      // To go from level L → L+1:
      //   1. Min total attempts : 5 × L × (L+1) / 2
      //        L=1 →  5,  L=2 → 15,  L=3 → 30,  L=4 → 50 …
      //   2. Window             : last (5 × L) attempts
      //        L=1 → last 5,  L=2 → last 10,  L=3 → last 15 …
      //   3. Required avg       : 4.0 + (L-1) × 0.1
      //        L=1 → 4.0,  L=2 → 4.1,  L=3 → 4.2 …  (no hard cap)
      // No upper level cap — levels are infinite.
      // ──────────────────────────────────────────────────────────────────────
...
// Level progress: how close the student is to leveling up
router.get("/level-progress", requireAuth(), async (req, res) => {
  const userId = req.user.id;
  const { rows: profileRows } = await query(
    `SELECT sp.global_level,
            (SELECT COUNT(*)::int FROM responses WHERE user_id=$1) AS total
  ...
  if (total > 0) {
    const { rows: avgRows } = await query(
      `SELECT COALESCE(AVG(score), 0)::float AS avg FROM (
         SELECT score FROM responses WHERE user_id=$1 ORDER BY created_at DESC LIMIT $2
       ) recent`,
      [userId, window]
    );
    recentAvg = parseFloat((avgRows[0]?.avg ?? 0).toFixed(2));
  }

  const attemptsPct = minAttempts > 0 ? Math.min(100, Math.round((total / minAttempts) * 100)) : 100;
  const scorePct    = requiredAvg > 0 ? Math.min(100, Math.round((recentAvg / requiredAvg) * 100)) : 0;
  ...
  res.json({
    ...
    scorePct,
    overallPct,
    attemptsReached: total >= minAttempts,
    scoreReached: recentAvg >= requiredAvg,
  });
});

// Level-matched cases: all cases in user's level range with attempt status
router.get("/level-cases", requireAuth(), async (req, res) => {
  const userId = req.user.id;
  const { rows: profile } = await query(
    `SELECT global_level FROM student_profiles WHERE user_id=$1`,
    [userId]
  );
  const L = profile[0]?.global_level ?? 1;
  const minLevel = Math.max(1, L - 1);
  const maxLevel = L + 1;
  ...
  const { rows } = await query(
    `SELECT c.id, c.title, c.specialty, c.level,
            (SELECT COUNT(*)::int FROM case_verifications WHERE case_id=c.id AND action='verify') AS verify_count,
            EXISTS(SELECT 1 FROM responses r WHERE r.user_id=$1 AND r.case_id=c.id) AS attempted,
            (SELECT AVG(r2.score)::float FROM responses r2 WHERE r2.user_id=$1 AND r2.case_id=c.id) AS my_avg
     FROM cases c
     WHERE c.deleted_at IS NULL AND c.level >= $2 AND c.level <= $3
     ORDER BY c.level ASC, attempted ASC, verify_count DESC, c.created_at DESC
     LIMIT $4 OFFSET $5`,
    [userId, minLevel, maxLevel, pageSize, offset]
  );

  const { rows: countRows } = await query(
    `SELECT COUNT(*)::int AS total FROM cases WHERE deleted_at IS NULL AND level >= $1 AND level <= $2`,
    [minLevel, maxLevel]
  );
  ...
  res.json({
    cases: rows.map((c) => ({
      ...
      myAvg: c.my_avg != null ? parseFloat(c.my_avg.toFixed(1)) : null,
    })),
