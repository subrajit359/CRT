...
// Returns any unread admin warnings the client should show as a center-screen popup.
router.get("/pending-warnings", requireAuth({ allowPending: true }), async (req, res) => {
  const { rows } = await query(
    `SELECT id, title, body, created_at
       FROM notifications
       WHERE user_id=$1 AND kind='warning' AND read_at IS NULL
