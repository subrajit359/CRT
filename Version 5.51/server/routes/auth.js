...
router.post("/register-student", async (req, res) => {
  try {
    ...
    await notify(rows[0].id, "welcome", "Welcome to CrLearn", "Start with the practice loop. One case, one question, one sharp evaluation.", "/practice");
...
router.post("/promote", async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: "Not signed in" });
    if (user.role === "admin") return res.status(400).json({ error: "Your account is already an admin" });
    if (!["student", "doctor"].includes(user.role)) return res.status(403).json({ error: "Not eligible for promotion" });

    const code = String(req.body.code || "").trim();
    if (!code) return res.status(400).json({ error: "Promo code is required" });

    const promoCode = process.env.PROMO_CODE;
    if (!promoCode) return res.status(503).json({ error: "Promotion feature is not configured" });

    if (code !== promoCode) return res.status(401).json({ error: "Invalid promo code" });

    await query(`UPDATE users SET role='admin' WHERE id=$1`, [user.id]);
    req.log?.info({ userId: user.id }, "User promoted to admin via promo code");
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
