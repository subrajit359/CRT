...
/* ── Contact messages ── */
router.get("/contact-messages", requireAuth(["admin"]), async (req, res) => {
  try {
    ...
    const { rows: countRows } = await query(`SELECT COUNT(*) AS total FROM contact_messages`);
    res.json({ items: rows, total: parseInt(countRows[0].total, 10), page, pageSize });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

router.delete("/contact-messages/:id", requireAuth(["admin"]), async (req, res) => {
  try {
...
router.get("/push/stats", requireAuth(["admin"]), async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT u.role, COUNT(ps.id)::int AS subs
         FROM push_subscriptions ps
         JOIN users u ON u.id = ps.user_id
         GROUP BY u.role`
    );
    const total = rows.reduce((s, r) => s + r.subs, 0);
    res.json({ total, byRole: rows });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Broadcast push notification to all or by role
router.post("/push/broadcast", requireAuth(["admin"]), async (req, res) => {
  try {
    const { title, body, link, role = "all" } = req.body || {};
    if (!title?.trim()) return res.status(400).json({ error: "title is required" });
    if (!body?.trim())  return res.status(400).json({ error: "body is required" });
    const result = await sendPushBroadcast(
      { title: title.trim(), body: body.trim(), link: link?.trim() || "/", kind: "announcement" },
      { role }
    );
    res.json(result);
  } catch (e) {
