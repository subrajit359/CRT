...
// Replit preview runs over HTTPS — cookies need secure+sameSite=none to survive the proxy
...
export function requireAuth(rolesOrOpts) {
  // Backwards-compatible: requireAuth(), requireAuth(["doctor"]), or
  // requireAuth({ roles: ["doctor"], allowPending: true }).
  // `allowPending: true` lets pending/rejected doctors hit the route — needed
  // for their own inbox status, notifications, and admin-chat endpoints.
  const opts =
  ...
  return async (req, res, next) => {
    ...
    // Enforce account ban (admins are never blocked)
