...
const SEED_DEFAULT_PASSWORD = "CrLearn#2026";
...
async function ensureDoctorProfile(userId, specialty) {
  await query(
    `INSERT INTO doctor_profiles (user_id, specialty, status, degree, years_exp, license_number, hospital)
     VALUES ($1,$2,'approved','MD',12,'SEED-0001','CrLearn Reference Hospital')
...
async function seed() {
  ...
  const adminId = await ensureUser({ email: ADMIN_EMAIL, username: ADMIN_USERNAME, fullName: "CrLearn Admin", role: "admin" });
  ...
  for (const c of SEED_CASES) {
    ...
    const { rows: ins } = await query(
      `INSERT INTO cases (title, specialty, level, body, questions, source, source_kind, uploader_id,
       ...
       VALUES ($1,$2,$3,$4,$5::jsonb,'CrLearn Library','admin',$6,$7,$8::jsonb,$9) RETURNING id`,
