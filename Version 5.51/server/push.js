...
// Broadcast push to all subscribed users (optionally filtered by role).
// role: "all" | "student" | "doctor" — defaults to "all"
export async function sendPushBroadcast(payload, { role = "all" } = {}) {
  try {
    await ensurePushConfigured();
    const roleFilter = role === "all" ? "" : `AND u.role = '${role}'`;
    const { rows } = await query(
    ...
    console.warn("[push] broadcast failed:", err?.message || err);
