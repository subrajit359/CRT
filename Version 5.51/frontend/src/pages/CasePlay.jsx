...
export default function CasePlay() {
  ...
  // The viewer is opened against the FULL list (images first, then docs) so
  // the prev/next arrows can flip between everything attached to the case.
  const viewerItems = [...images, ...docs];
  ...
  return (
                  ...
                  <ul className="att-doclist">
                    {docs.map((a, i) => (
                      <li key={a.id}>
                        ...
                        <span className="muted small"> · {a.size_bytes ? `${(a.size_bytes / 1024).toFixed(0)} KB` : (a.mime_type || "file")}</span>
                      </li>
                    ))}
                  </ul>
                )}
    ...
    />
    {/* AttachmentViewer must live OUTSIDE motion.div — Framer Motion applies
        CSS transforms during animation which creates a stacking context that
        traps position:fixed children and prevents them from covering the
        app header. Rendering it as a sibling of AppShell avoids this. */}
    {lightboxIndex !== null && viewerItems[lightboxIndex] && (
      <AttachmentViewer
        items={viewerItems}
        index={lightboxIndex}
