...
// Bullet icon for each password-strength rule. `required` rules show an X when
// failing (red); recommended rules show an empty circle (neutral) so the
// reader can immediately tell which ones are blocking submission.
function PwRuleIcon({ ok, required }) {
  const Icon = ok ? Check : (required ? X : Circle);
  return (
    <Icon
      size={14}
      strokeWidth={2}
  ...
  async function submit(e) {
    ...
    try {
      if (role === "student") {
        ...
        await refresh();
        toast.success("Welcome to CrLearn");
        navigate("/");
      } else {
        ...
        <div className="auth-side">
          <div>
            <span className="hero-eyebrow" style={{ background: "rgba(255,255,255,0.08)", borderColor: "rgba(255,255,255,0.16)", color: "white" }}>
              <span className="dot" /> Join CrLearn
            </span>
                  ...
                  {password && (
                    <>
                      ...
                      <ul className="pw-rules">
                        <li className={pwRules.length ? "ok" : "bad"}><PwRuleIcon ok={pwRules.length} required /> At least 8 characters</li>
                        <li className={pwRules.letter ? "ok" : "bad"}><PwRuleIcon ok={pwRules.letter} required /> Contains a letter</li>
                        <li className={pwRules.number ? "ok" : "bad"}><PwRuleIcon ok={pwRules.number} required /> Contains a number</li>
                        <li className={pwRules.upper ? "ok" : "soft"}><PwRuleIcon ok={pwRules.upper} /> Uppercase letter (recommended)</li>
                        <li className={pwRules.special ? "ok" : "soft"}><PwRuleIcon ok={pwRules.special} /> Special character (recommended)</li>
                      </ul>
                    </>
                  )}
                  {!password && (
