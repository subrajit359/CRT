...
export default function PracticeStart() {
  ...
  // When BOTH specialty + level are picked, load the group breakdown for this student.
  useEffect(() => {
    if (!specialty || !level) { setGroupsData(null); return; }
    let cancelled = false;
    setLoadingGroups(true);
    setGroupsData(null);
    const params = new URLSearchParams({ specialty, level });
  ...
  function openGroup(group) {
    ...
    if (!target) return;
    const qs = new URLSearchParams({ specialty, level: String(level), group: String(group.index) });
      ...
      <div className="container fade-in" style={{ maxWidth: 920 }}>
        ...
        <div className="card">
          ...
          <div className="muted small" style={{ marginTop: 8, textAlign: "center" }}>
            Tip: choose a specialty <em>and</em> level to unlock group practice below.
          </div>
        </div>
    ...
    return (
      <div className="card">
        ...
        <p className="muted" style={{ marginBottom: 0 }}>
          There are no cases in <strong>{specialty}</strong> · Level {level} yet. Try a different combination, or use the random pick above.
        </p>
      </div>
    );
  }
    ...
    >
      <div className="card" style={{ background: "linear-gradient(135deg, rgba(200,169,106,0.10), rgba(200,169,106,0.02))" }}>
        <div className="row-between" style={{ alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <div>
            <h3 style={{ margin: 0 }}>{specialty} · Level {level}</h3>
