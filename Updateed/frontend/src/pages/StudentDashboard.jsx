...
function milestoneLabel(n) {
  if (n >= 100) return `${n}-day streak — Iron habit 🏛️`;
  if (n >= 60) return `${n}-day streak — Marathon mind 🧠`;
  if (n >= 30) return `${n}-day streak — One full month 🗓️`;
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      api.get("/api/eval/stats").catch(() => null),
      api.get("/api/eval/next").catch(() => ({ case: null })),
      api.get("/api/eval/changes").catch(() => ({ events: [] })),
      api.get("/api/leaderboard?period=all&page=1&pageSize=1").catch(() => null),
    ]).then(([s, n, c, lb]) => {
      if (!alive) return;
      setStats(s);
      setNext(n);
            ...
            <div>
              ...
              <div className="today-meta">
                ...
                {next.case.verify_count > 0 && (
                  <span className="badge">✓ {next.case.verify_count} doctor{next.case.verify_count > 1 ? "s" : ""}</span>
                )}
              </div>
...
function RankCard({ rankInfo, loading, onOpen }) {
  ...
  if (delta != null) {
    if (delta > 0) deltaNode = <span style={{ color: "var(--green-700, #0F4C3A)", fontWeight: 700 }}>▲ {delta} this week</span>;
