...
router.post("/", requireAuth(), async (req, res) => {
  try {
    ...
    // 1) Deterministic diagnosis verdict — the trustworthy part. No AI involved.
    const dx = matchDiagnosis(userAnswer, c.diagnosis, c.accepted_diagnoses);
    ...
    if (isPractice && cachedEvalText) {
      ...
      try {
        const resp = await openai.chat.completions.create({
          model: process.env.AI_EVAL_MODEL || process.env.AI_MODEL || "gemini-2.5-flash",
