...
// Bulk uploads attach files per-case using field names like `case_1`, `case_2`, …
// Allow more total files than the single-case limit since one batch can hold many cases.
...
router.post("/bulk", requireAuth(["doctor", "admin"]), bulkUpload.any(), async (req, res) => {
  try {
    ...
    // Group uploaded files by their per-case field name: `case_1`, `case_2`, …
    // Files outside that pattern are ignored.
    const filesByCaseIndex = new Map(); // 1-based index -> File[]
    for (const f of uploadedFiles) {
      const m = /^case_(\d+)$/i.exec(String(f.fieldname || ""));
      if (!m) continue;
      const idx = parseInt(m[1], 10);
      if (!filesByCaseIndex.has(idx)) filesByCaseIndex.set(idx, []);
      filesByCaseIndex.get(idx).push(f);
    }
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      try {
        ...
        // Attach the files the user picked under this case's file input (if any).
        let attachedCount = 0;
        const caseFiles = filesByCaseIndex.get(i + 1) || [];
        if (caseFiles.length) {
          if (!cloudinaryReady()) {
            ...
            for (const file of caseFiles) {
              try {
                ...
                attachmentWarnings.push({ index: i + 1, title, warning: `Failed to attach ${file.originalname}: ${attErr.message}` });
              }
            }
          }
        }
