...
// Distinct specialties students have actually attempted — used by the UI to
// build the per-specialty ranking filter.
router.get("/specialties", requireAuth(), async (req, res) => {
  const { rows } = await query(
    `SELECT DISTINCT c.specialty
       ...
       WHERE c.specialty IS NOT NULL
       ORDER BY c.specialty ASC`
  );
...
router.get("/", requireAuth(), async (req, res) => {
  ...
  // category: streakScore (default — overall) | avgScore | streakDays | attempts | specialty
  const category = (req.query.category || "streakScore").toString();
  ...
  // Cases done per user (within period) + overall avg score
  const { rows: casesRows } = await query(
    `SELECT r.user_id,
            COUNT(DISTINCT r.case_id)::int AS n,
            AVG(r.score)::float            AS avg_score,
            COUNT(r.score)::int            AS scored_n
  ...
  for (const r of casesRows) {
    casesByUser.set(r.user_id, r.n);
    avgByUser.set(r.user_id, { avg: r.avg_score, n: r.scored_n });
  }

  // Per-specialty stats per user (within period). Used for top-specialty label
  // AND for the per-specialty ranking category.
  const { rows: specRows } = await query(
    `SELECT r.user_id, c.specialty,
            COUNT(*)::int    AS n,
            AVG(r.score)::float AS avg_score
       FROM responses r JOIN cases c ON c.id = r.case_id
       WHERE r.user_id = ANY($1) ${intervalClause}
  ...
  const specStatsByUser = new Map(); // userId -> { specialty -> {n, avg} }
  for (const r of specRows) {
    const cur = topSpecByUser.get(r.user_id);
    if (!cur || r.n > cur.n) topSpecByUser.set(r.user_id, r.specialty);
    if (!specStatsByUser.has(r.user_id)) specStatsByUser.set(r.user_id, {});
    specStatsByUser.get(r.user_id)[r.specialty] = { n: r.n, avg: r.avg_score };
  }
  ...
  for (const u of users) {
    ...
    const av = avgByUser.get(u.id);
    const specStats = specStatsByUser.get(u.id) || {};
    const specEntry = specialtyFilter ? specStats[specialtyFilter] : null;
    entries.push({
      userId: u.id,
      name: u.full_name,
      username: u.username,
      ...
      score: Math.round(s.score * 10) / 10, // streak-strength score
      avgScore: av && av.n > 0 ? Math.round(av.avg * 10) / 10 : null,
      avgScoreCount: av ? av.n : 0,
      specialtyAvg: specEntry ? Math.round(specEntry.avg * 10) / 10 : null,
      specialtyAttempts: specEntry ? specEntry.n : 0,
      tier: tierFor(s.score),
      joinedAt: u.created_at,
    });
  }

  // Filter for the per-specialty board: only users who actually attempted that specialty.
  if (category === "specialty" && specialtyFilter) {
    entries = entries.filter((e) => e.specialtyAttempts > 0);
  }

  // Sort by category. Ties always fall back to overall streak score so rankings stay stable.
  function bySort(getter, tieBreak = (e) => e.score) {
    return (a, b) => (getter(b) - getter(a)) || (tieBreak(b) - tieBreak(a)) || (b.casesCompleted - a.casesCompleted);
  }
  if (category === "avgScore") {
    entries = entries.filter((e) => e.avgScore != null && e.avgScoreCount >= 3);
    entries.sort(bySort((e) => e.avgScore));
  } else if (category === "streakDays") {
    entries.sort(bySort((e) => e.bestStreak, (e) => e.currentStreak));
  } else if (category === "attempts") {
    entries.sort(bySort((e) => e.casesCompleted));
  } else if (category === "specialty" && specialtyFilter) {
    entries.sort(bySort((e) => e.specialtyAvg ?? -1, (e) => e.specialtyAttempts));
  } else {
    // streakScore (default)
    entries.sort((a, b) =>
      b.score - a.score ||
