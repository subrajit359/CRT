...
router.get("/me", requireAuth({ roles: ["doctor"], allowPending: true }), async (req, res) => {
  ...
  res.json({ profile, thread: { id: threadId }, messages: msgs });
});
...
router.post("/me/messages", requireAuth({ roles: ["doctor"], allowPending: true }), async (req, res) => {
  ...
  const threadId = await getOrCreateThread(me);
  const { rows: ins } = await query(
    `INSERT INTO support_messages (thread_id, sender_id, body, kind)
       VALUES ($1, $2, $3, 'text')
     RETURNING id`,
    [threadId, me, body]
  );
  ...
  const { rows: msg } = await query(`${MESSAGE_SELECT} WHERE m.id = $1`, [ins[0].id]);
...
router.post("/:threadId/reapply-invite", requireAuth(["admin"]), async (req, res) => {
  ...
  const { rows: ins } = await query(
    `INSERT INTO support_messages (thread_id, sender_id, body, kind, meta)
       VALUES ($1, $2, $3, 'reapply_invite', $4::jsonb)
     RETURNING id`,
    [t[0].id, me, adminBody, JSON.stringify({ note: note || null })]
  );
  await query(`UPDATE support_threads SET last_at = NOW() WHERE id = $1`, [t[0].id]);
  await markRead(me, t[0].id);
